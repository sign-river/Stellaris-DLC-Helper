# Stellaris DLC Helper - 开发者文档

## 目录

- [项目架构](#项目架构)
- [开发环境搭建](#开发环境搭建)
- [核心模块说明](#核心模块说明)
- [测速系统详解](#测速系统详解)
- [打包部署](#打包部署)
- [配置管理](#配置管理)
- [错误处理](#错误处理)
- [开发规范](#开发规范)

---

## 项目架构

### 目录结构

```
Stellaris-DLC-Helper/
├── main.py                 # 程序入口
├── build.py               # 打包脚本
├── config.json            # 程序配置（包含在仓库中）
├── pairings.json          # 文件映射配置
├── requirements.txt       # 依赖列表
├── src/                   # 源代码
│   ├── __init__.py
│   ├── config.py         # 配置管理
│   ├── config_loader.py  # 配置加载器
│   ├── core/             # 核心功能
│   │   ├── dlc_manager.py      # DLC管理
│   │   ├── downloader.py       # 下载器
│   │   ├── installer.py        # 安装器
│   │   ├── patch_manager.py    # 补丁管理
│   │   ├── source_manager.py   # 多源管理
│   │   ├── speed_test.py       # 测速模块
│   │   └── updater.py          # 自动更新
│   ├── gui/              # 用户界面
│   │   ├── main_window.py      # 主窗口
│   │   ├── settings_dialog.py  # 设置对话框
│   │   └── update_dialog.py    # 更新对话框
│   └── utils/            # 工具模块
│       ├── error_handler.py    # 错误处理
│       ├── logger.py           # 日志系统
│       ├── path_utils.py       # 路径工具
│       └── steam_utils.py      # Steam工具
├── patches/              # 补丁文件
├── assets/               # 资源文件
│   └── images/          # 图片资源
├── docs/                # 文档
└── tools/               # 开发工具
```

### 技术栈

- **Python 3.8+**
- **GUI框架**: CustomTkinter
- **HTTP请求**: requests
- **打包工具**: PyInstaller

---

## 开发环境搭建

### 1. 克隆项目

```bash
git clone https://github.com/sign-river/Stellaris-DLC-Helper.git
cd Stellaris-DLC-Helper
```

### 2. 创建虚拟环境

```bash
python -m venv venv

# Windows
venv\Scripts\activate

# Linux/Mac
source venv/bin/activate
```

### 3. 安装依赖

```bash
pip install -r requirements.txt
```

### 4. 运行程序

```bash
python main.py
```

### 5. 开发工具

推荐使用 VS Code + Python 扩展：

```json
// .vscode/settings.json
{
    "python.linting.enabled": true,
    "python.linting.pylintEnabled": true,
    "python.formatting.provider": "black"
}
```

---

## 核心模块说明

### 1. DLC Manager (dlc_manager.py)

**职责**：管理 DLC 列表、获取 DLC 信息

**核心方法**：
- `fetch_dlc_list()`: 从服务器获取 DLC 列表
- `get_dlc_info(dlc_key)`: 获取特定 DLC 信息
- `merge_dlc_data()`: 合并多源 DLC 数据

**使用示例**：
```python
from src.core import DLCManager

manager = DLCManager()
dlc_list = manager.fetch_dlc_list()
```

### 2. Source Manager (source_manager.py)

**职责**：管理多个下载源，选择最优源

**核心方法**：
- `get_best_download_source()`: 测速选择最佳源
- `measure_speed()`: 测试单个源的速度
- `get_download_urls_for_dlc()`: 获取 DLC 的所有下载URL

**阈值配置**：
```python
# 三级阈值策略
THRESHOLDS = {
    "r2": 2.5,           # 海外源：筛选有梯子用户
    "github": 2.5,       # 海外源：筛选有梯子用户
    "domestic_cloud": 3.0,  # 国内云：避免拥挤时段
    "gitee": 0.8         # Gitee：稳定保底
}
```

### 3. Downloader (downloader.py)

**职责**：处理文件下载，支持断点续传

**核心方法**：
- `download_file()`: 下载单个文件
- `pause()`: 暂停下载
- `resume()`: 恢复下载
- `stop()`: 停止下载

**特性**：
- 断点续传支持
- 进度回调
- 速度监控
- 错误重试

### 4. Updater (updater.py)

**职责**：自动更新功能

**核心方法**：
- `check_for_updates()`: 检查更新
- `fetch_announcement()`: 获取公告
- `download_update()`: 下载更新
- `apply_update()`: 应用更新

**更新流程**：
```python
# 1. 检查更新
update_info, announcement = updater.check_for_updates()

# 2. 下载更新
if update_info and update_info.has_update:
    updater.download_update(update_info, progress_callback)

# 3. 应用更新
updater.apply_update(update_file_path)
```

---

## 测速系统详解

### 三层选源设计

采用三层智能选源策略，确保用户总能获得可用的下载源：

```
【第一层：优选快速源】🚀
  测试所有源 → 选择达到阈值的最快源
  - R2/GitHub: 2.5 MB/s  ← 筛选有梯子的用户
  - 国内云: 3.0 MB/s     ← 避开拥挤时段
  - Gitee: 不参与此层    ← 保留测速展示，作为保底
  
  ✅ 有达标源 → 返回最快的达标源

【第二层：默认源保底】⚠️
  ❌ 所有源都不达标 → 使用国内云
  - 面向国内用户的默认选择
  - 配置问题时的安全选项
  
  ✅ 返回国内云

【第三层：连接保底】💾
  ❌ 连国内云都无法连接 → 使用Gitee
  - Gitee虽慢但非常稳定
  - 最后的可靠保障
  
  ✅ 返回Gitee
```

### 阈值设计理念

每个源的阈值反映其使用场景：

| 源 | 阈值 | 理由 |
|----|------|------|
| R2/GitHub | 2.5 MB/s | 需梯子用户才能用，所以设高 → 筛选出真正有梯子的 |
| 国内云 | 3.0 MB/s | 国内默认源，但可能拥挤 → 高阈值筛选空闲时段 |
| Gitee | —（不参与） | 保底源，只记录速度 → 不需要阈值判断 |

### 测速实现

```python
def measure_speed(self, url, description, threshold_mb, 
                 log_callback=None, max_seconds=10.0, 
                 max_bytes=100*1024*1024):
    """
    测速单个URL - 通过实际下载测试真实速度
    
    参数:
        url: 测试URL
        description: 描述信息
        threshold_mb: 速度阈值(MB/s)
                     特殊值: -1 表示保底源（Gitee），
                     只记录速度不做判断
        log_callback: 日志回调函数
        max_seconds: 最大测试时间（默认10秒）
        max_bytes: 最大下载字节数（默认100MB）
    
    返回:
        tuple: (是否达标, 速度MB/s)
    """
    # 10秒实际下载测试
    start_time = time.time()
    total_downloaded = 0
    last_report_time = time.time()
    report_interval = 2.0  # 每2秒报告一次
    
    # 下载循环
    for chunk in response.iter_content(chunk_size=64*1024):
        total_downloaded += len(chunk)
        current_time = time.time()
        
        # 实时进度报告
        if current_time - last_report_time >= report_interval:
            current_speed = (total_downloaded/1024/1024) / (current_time-start_time)
            log_callback(f"测速中... {current_speed:.2f} MB/s")
            last_report_time = current_time
        
        # 停止条件
        if current_time - start_time >= max_seconds:
            break
    
    # 计算结果
    speed_mb = (total_downloaded/1024/1024) / (time.time()-start_time)
    
    # 判断是否达标
    if threshold_mb < 0:  # 保底源（Gitee）
        return True, speed_mb  # 只记录速度，总是返回True
    else:
        return speed_mb > threshold_mb, speed_mb  # 普通源进行阈值判断
```

### 选源逻辑（get_best_download_source）

```python
def get_best_download_source(self, silent=False, log_callback=None):
    """测速选择最佳源（带三层保障）"""
    
    # 第一步：测试所有源
    test_results = {}  # {source: (speed, url)}
    domestic_cloud_data = None  # 默认源备用
    gitee_data = None  # 保底源备用
    
    for source_name in ["r2", "github", "domestic_cloud", "gitee"]:
        # 根据源设置不同的阈值
        if source_name in ["r2", "github"]:
            threshold = 2.5
        elif source_name == "domestic_cloud":
            threshold = 3.0
        else:  # gitee
            threshold = -1  # 保底源，只记录不判断
        
        ok, speed = self.measure_speed(url, source_name, threshold)
        
        if source_name == "gitee":
            gitee_data = (speed, url)
        elif source_name == "domestic_cloud":
            domestic_cloud_data = (speed, url)
        
        if ok:  # 达标源记录
            test_results[source_name] = (speed, url)
    
    # 第二步：选择逻辑
    if test_results:
        # 第一层：有达标源，选最快的
        best = max(test_results.items(), key=lambda x: x[1][0])
        return best[0], best[1][0]
    elif domestic_cloud_data:
        # 第二层：都不达标，用默认源
        return "domestic_cloud", domestic_cloud_data[1]
    elif gitee_data:
        # 第三层：连默认源都没数据，用保底源
        return "gitee", gitee_data[1]
    else:
        # 极端情况：完全无法连接
        return "gitee", HARDCODED_FALLBACK_URL
```

### 智能缓存

```python
class SourceManager:
    def __init__(self):
        self._speed_cache = {}           # {source_name: (speed, timestamp)}
        self._cache_validity = 300       # 5分钟有效期
        self._last_best_source = None    
        self._last_best_timestamp = 0
    
    def get_best_download_source(self, force_retest=False):
        """测速选择最佳源（带缓存）"""
        current_time = time.time()
        
        # 检查缓存
        if not force_retest and self._last_best_source:
            cache_age = current_time - self._last_best_timestamp
            if cache_age < self._cache_validity:
                # 使用缓存
                return self._cached_result
        
        # 执行测速...
        best_source = self._do_speed_test()
        
        # 更新缓存
        self._last_best_source = best_source
        self._last_best_timestamp = current_time
        
        return best_source
```

### 防止循环测速

在下载过程中的速度检测：

```python
# 严格的触发条件（AND关系）
if (not self.download_paused and          # 未暂停
    download_duration > 10.0 and          # 下载超过10秒
    display_speed < 0.3 and               # 速度 < 0.3 MB/s
    downloaded > 5 * 1024 * 1024):        # 已下载 > 5MB
    
    slow_speed_count += 1
    
    # 连续5次慢速才触发
    if slow_speed_count >= 5 and not server_issue_detected:
        # 每30秒最多检查一次
        if current_time - last_server_check >= 30:
            retest_and_switch_source()
```

---

## 打包部署

### 平台兼容性

- ✅ Windows 10/11
- ✅ Windows 7 (需要特殊处理)
- ⚠️ Linux (理论支持，需测试)
- ⚠️ macOS (理论支持，需测试)

### Windows 打包

#### 方式1：使用 build.py（推荐）

```bash
python build.py
```

生成文件：`dist/Stellaris_DLC_Helper.exe`

#### 方式2：手动打包

```bash
pip install -r requirements-build.txt

pyinstaller --name="Stellaris_DLC_Helper" \
    --onefile \
    --windowed \
    --icon=assets/images/tea_Gray.ico \
    --add-data "assets;assets" \
    --add-data "patches;patches" \
    --add-data "pairings.json;." \
    --hidden-import=customtkinter \
    main.py
```

### 打包配置

`build.py` 关键配置：

```python
# PyInstaller 配置
a = Analysis(
    ['main.py'],
    pathex=[],
    binaries=[],
    datas=[
        ('assets', 'assets'),
        ('patches', 'patches'),
        ('pairings.json', '.'),
    ],
    hiddenimports=['customtkinter'],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[],
    win_no_prefer_redirects=False,
    win_private_assemblies=False,
    cipher=None,
    noarchive=False,
)
```

### Windows 7 兼容性

需要特殊处理 asyncio：

```python
# build.py 中添加
import sys
if sys.platform == 'win32':
    import asyncio
    asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
```

### 发布流程

1. **更新版本号**：修改 `src/config.py` 中的 `VERSION`
2. **测试程序**：确保所有功能正常
3. **打包程序**：运行 `python build.py`
4. **测试EXE**：在干净环境测试
5. **上传服务器**：
   ```bash
   # 更新 version.json
   {
       "version": "1.1.0",
       "download_url": "https://xxx/Stellaris_DLC_Helper_v1.1.0.exe",
       "changelog": "1. 优化测速...\n2. 修复..."
   }
   ```
6. **发布 Release**：在 GitHub 创建 Release

---

## 配置管理

### config.json 结构

```json
{
    "game_path": "C:/Program Files/Steam/steamapps/common/Stellaris",
    "theme": "light",
    "auto_update": true,
    "download": {
        "chunk_size": 65536,
        "max_retries": 3,
        "timeout": 30
    },
    "sources": {
        "enabled": ["r2", "github", "domestic_cloud", "gitee"],
        "priority": ["r2", "github", "domestic_cloud", "gitee"]
    }
}
```

### 配置加载

```python
from src.config_loader import ConfigLoader

config = ConfigLoader()
game_path = config.get('game_path')
config.set('theme', 'dark')
config.save()
```

### 多源配置

在 `src/config.py` 中配置：

```python
DLC_SOURCES = [
    {
        "name": "r2",
        "url": "https://dlc.dlchelper.top",
        "format": "standard",
        "enabled": True,
        "priority": 1,
        "test_url": "https://dlc.dlchelper.top/test/test2.bin",
        "threshold_mb": 2.5
    },
    # ... 其他源配置
]
```

---

## 错误处理

### 错误类型

```python
ERROR_TYPES = {
    'network': '网络连接失败',
    'disk': '磁盘空间不足',
    'permission': '权限不足',
    'file': '文件操作失败',
    'unknown': '未知错误'
}
```

### 错误处理装饰器

```python
from src.utils.error_handler import handle_errors

@handle_errors(error_type='network', context='下载文件')
def download_file(url, path):
    # 下载逻辑
    pass
```

### 用户友好提示

```python
def show_friendly_error(error_type, exception, context=""):
    """显示友好的错误提示"""
    error_messages = {
        'network': {
            'title': '网络连接失败',
            'message': '无法连接到服务器\n\n可能的原因：\n...'
        },
        # ...
    }
    messagebox.showerror(title, message)
```

---

## 开发规范

### 代码风格

遵循 PEP 8 规范：

```python
# 好的命名
def download_file(url: str, path: str) -> bool:
    """下载文件到指定路径"""
    pass

# 类型注解
from typing import List, Dict, Optional

def process_dlc_list(dlcs: List[Dict[str, Any]]) -> Optional[str]:
    pass
```

### 注释规范

```python
def complex_function(param1, param2):
    """
    函数简短描述
    
    详细描述功能和使用场景...
    
    参数:
        param1: 参数1说明
        param2: 参数2说明
    
    返回:
        返回值说明
    
    异常:
        可能抛出的异常
    
    示例:
        >>> complex_function("test", 123)
        "result"
    """
    pass
```

### Git 提交规范

```bash
# 提交格式
<type>(<scope>): <subject>

# 类型
feat:     新功能
fix:      修复bug
docs:     文档更新
style:    代码格式
refactor: 重构
test:     测试
chore:    构建/工具

# 示例
git commit -m "feat(downloader): 添加断点续传功能"
git commit -m "fix(updater): 修复更新失败bug"
git commit -m "docs(readme): 更新使用说明"
```

### 测试规范

```python
# tools/test_*.py
import unittest

class TestDownloader(unittest.TestCase):
    def setUp(self):
        """测试前准备"""
        self.downloader = Downloader()
    
    def test_download_file(self):
        """测试文件下载"""
        result = self.downloader.download_file(test_url, test_path)
        self.assertTrue(result)
    
    def tearDown(self):
        """测试后清理"""
        pass

if __name__ == '__main__':
    unittest.main()
```

---

## API文档

### DLCManager API

```python
class DLCManager:
    def fetch_dlc_list(self) -> List[Dict[str, Any]]:
        """获取DLC列表"""
        
    def get_dlc_info(self, dlc_key: str) -> Optional[Dict[str, Any]]:
        """获取特定DLC信息"""
```

### SourceManager API

```python
class SourceManager:
    def get_best_download_source(self, 
                                 silent: bool = False,
                                 log_callback: Callable = None,
                                 force_retest: bool = False
                                ) -> Tuple[str, str]:
        """
        测速选择最佳下载源
        
        返回: (源名称, 测试URL)
        """
    
    def measure_speed(self,
                     url: str,
                     description: str,
                     threshold_mb: float,
                     log_callback: Callable = None,
                     max_seconds: float = 10.0
                    ) -> Tuple[bool, float]:
        """
        测速单个URL
        
        返回: (是否达标, 速度MB/s)
        """
```

---

## 贡献指南

### 如何贡献

1. Fork 项目
2. 创建功能分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'feat: Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

### 代码审查

PR 需要满足：
- ✅ 通过所有测试
- ✅ 遵循代码规范
- ✅ 包含必要的文档
- ✅ 无明显性能问题

---

## 联系方式

- **GitHub**: https://github.com/sign-river/Stellaris-DLC-Helper
- **Issues**: https://github.com/sign-river/Stellaris-DLC-Helper/issues
- **QQ群**: 1051774780
