...
本文件合并了项目中所有关于多源下载逻辑与实现细节，包含R2、国内云、GitHub、Gitee四个源的生成规则、优先级、测速和断点续传等内容。

（本文件整合自原 `DOWNLOAD_LOGIC_DETAIL.md` 与 `MULTI_SOURCE_README.md`，保留核心实现细节）

## 概要
见 `DOWNLOAD_LOGIC_DETAIL.md` 的核心说明：项目实现多源下载与智能故障转移，根据优先级尝试下载源，并支持多种格式的URL映射。

## 源与优先级
- R2：优先级1，主要下载源，使用 R2 存储桶直接提供文件
- 国内云服务器：优先级2，唯一的DLC列表来源，`index.json`在此服务器中
- GitHub：优先级3，使用 `pairings.json` 进行文件名映射并发布至 release
...
---
注：源文件合并后，原始单独文档已合并到该文件并存档。若需要保留英文版本或原始文档，请从 `docs/legacy/` 恢复。
